...
-/
import BridgelandStabLean.StabilityCondition.Symmetry.GLTilde.Action.Slicing
import Mathlib.CategoryTheory.Triangulated.Functor
import Mathlib.CategoryTheory.Shift.Adjunction

/-!
# Transporting slicings along triangulated autoequivalences

This is the categorical half of the symmetry action. `G̃L⁺(2, ℝ)` acts by
...
## Assembly into the stability-condition action

The sibling modules add the three structures needed above slicing transport:

1. `K₀` functoriality via `K₀.lift` and triangle additivity;
2. a compatible automorphism of the class lattice;
3. invariance of strict finite length under equivalences of interval categories.

The third point differs from phase relabelling: endpoints do not move, so no
