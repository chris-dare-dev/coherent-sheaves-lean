...
cover computes `Sheaf.H` in all degrees. On the small site of opens, neither the resolution nor
Mathlib's `HasExt` witness is an input: the pinned Mathlib makes abelian sheaves over a small site
a Grothendieck abelian category, hence one with enough injectives, and the comparison is stated
with no witness arguments. The witness-carrying form is retained for a general site, where enough
injectives is genuinely unavailable.
