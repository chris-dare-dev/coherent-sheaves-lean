...
It exists to keep `scripts/BridgelandAudit.lean`'s docstring honest. That comment states
...
This is a script, NOT a gate and NOT a build target. `lean_lib "BridgelandAudit"`
roots at `BridgelandAudit`, so a sibling file in `scripts/` is not swept into it. It reports the
...
run_cmd do
  ...
  -- names scripts/BridgelandAudit.lean actually gates
  let auditSrc ← IO.FS.readFile "scripts/BridgelandAudit.lean"
