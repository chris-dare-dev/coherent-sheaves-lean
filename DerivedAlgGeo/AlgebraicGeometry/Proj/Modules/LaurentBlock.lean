...
noncomputable def AwayRep.blockFilter {γ : ι →₀ ℕ} {d : ℕ} (F : Finset ι)
    (r : AwayRep ι R γ d) : AwayRep ι R γ d where
  pow := r.pow
  num := laurentFilter γ r.pow F r.num
  num_mem := laurentFilter_mem_natShift r.num_mem
...
theorem AwayRep.frac_blockFilter_raise {γ : ι →₀ ℕ} {d : ℕ} (F : Finset ι)
    (r : AwayRep ι R γ d) (t : ℕ) :
    (r.blockFilter F).frac =
      DegreeZeroLocalization.awayMk (𝓜 := natShift (polynomialGrading ι R) d)
        ...
        (laurentFilter_mem_natShift (r.pow_mul_num_mem t)) := by
  ...
  have hmem2 : (MvPolynomial.monomial γ (1 : R)) ^ t • laurentFilter γ r.pow F r.num ∈
    ...
    exact laurentFilter_mem_natShift (r.pow_mul_num_mem t)
  have hcongr : ∀ (a b : MvPolynomial ι R)
      (ha : a ∈ natShift (polynomialGrading ι R) d ((r.pow + t) • γ.degree))
      (hb : b ∈ natShift (polynomialGrading ι R) d ((r.pow + t) • γ.degree)), a = b →
      DegreeZeroLocalization.awayMk (𝓜 := natShift (polynomialGrading ι R) d)
  ...
  exact DegreeZeroLocalization.awayMk_shift
    (monomial_one_mem_polynomialGrading (R := R) γ) r.pow t
    (laurentFilter γ r.pow F r.num) (laurentFilter_mem_natShift r.num_mem) hmem2

set_option maxHeartbeats 800000 in
/-- **Well-definedness.** Two representatives of one fraction filter to the same fraction. -/
theorem AwayRep.frac_blockFilter_congr [IsDomain R] {γ : ι →₀ ℕ} {d : ℕ} (F : Finset ι)
    {r₁ r₂ : AwayRep ι R γ d} (h : r₁.frac = r₂.frac) :
    (r₁.blockFilter F).frac = (r₂.blockFilter F).frac := by
  ...
  refine (r₁.frac_blockFilter_raise F r₂.pow).trans
    (Eq.trans (hgen _ _ _ _ _ _ (Nat.add_comm _ _) ?_)
      (r₂.frac_blockFilter_raise F r₁.pow).symm)
...
should use. -/
noncomputable def blockProj [IsDomain R] (γ : ι →₀ ℕ) (F : Finset ι) (d : ℕ)
    ...
    DegreeZeroLocalization (polynomialGrading ι R)
      (natShift (polynomialGrading ι R) d) (.powers (MvPolynomial.monomial γ (1 : R))) :=
  ((AwayRep.frac_surjective γ d z).choose.blockFilter F).frac

theorem blockProj_frac [IsDomain R] (γ : ι →₀ ℕ) (F : Finset ι) (d : ℕ)
    (r : AwayRep ι R γ d) :
    blockProj γ F d r.frac = (r.blockFilter F).frac :=
  AwayRep.frac_blockFilter_congr F (AwayRep.frac_surjective γ d r.frac).choose_spec

/-- **The defining equation of the block projection**: on a representative it filters the
numerator and keeps the exponent. -/
theorem blockProj_awayMk [IsDomain R] (γ : ι →₀ ℕ) (F : Finset ι) {d m : ℕ}
    {p : MvPolynomial ι R}
    (hp : p ∈ natShift (polynomialGrading ι R) d (m • γ.degree)) :
    blockProj γ F d
        (DegreeZeroLocalization.awayMk (𝓜 := natShift (polynomialGrading ι R) d)
      ...
      DegreeZeroLocalization.awayMk (𝓜 := natShift (polynomialGrading ι R) d)
        ...
        (laurentFilter_mem_natShift hp) :=
  blockProj_frac γ F d ⟨m, p, hp⟩

set_option maxHeartbeats 800000 in
/-- The block projection is additive: align the fractions at a common denominator, where it is
`laurentFilter_add`. -/
theorem blockProj_add [IsDomain R] (γ : ι →₀ ℕ) (F : Finset ι) (d : ℕ)
    (z₁ z₂ : DegreeZeroLocalization (polynomialGrading ι R)
      (natShift (polynomialGrading ι R) d) (.powers (MvPolynomial.monomial γ (1 : R)))) :
    blockProj γ F d (z₁ + z₂) = blockProj γ F d z₁ + blockProj γ F d z₂ := by
  ...
  rw [← DegreeZeroLocalization.awayMk_add
    ...
    blockProj_awayMk γ F, blockProj_awayMk γ F, blockProj_awayMk γ F,
    ← DegreeZeroLocalization.awayMk_add (monomial_one_mem_polynomialGrading (R := R) γ) m _ _
      (laurentFilter_mem_natShift hp₁) (laurentFilter_mem_natShift hp₂)]
  have hcongr : ∀ (a b : MvPolynomial ι R)
...
noncomputable def blockProjHom [IsDomain R] (γ : ι →₀ ℕ) (F : Finset ι) (d : ℕ) :
    DegreeZeroLocalization (polynomialGrading ι R)
      ...
      DegreeZeroLocalization (polynomialGrading ι R)
        (natShift (polynomialGrading ι R) d)
        (.powers (MvPolynomial.monomial γ (1 : R))) :=
  AddMonoidHom.mk' (blockProj γ F d) (blockProj_add γ F d)

@[simp]
theorem blockProjHom_apply [IsDomain R] (γ : ι →₀ ℕ) (F : Finset ι) (d : ℕ)
    (z : DegreeZeroLocalization (polynomialGrading ι R)
      (natShift (polynomialGrading ι R) d) (.powers (MvPolynomial.monomial γ (1 : R)))) :
    blockProjHom γ F d z = blockProj γ F d z :=
  rfl
...
theorem sum_blockProj [IsDomain R] (γ : ι →₀ ℕ) (d : ℕ)
    (z : DegreeZeroLocalization (polynomialGrading ι R)
      (natShift (polynomialGrading ι R) d) (.powers (MvPolynomial.monomial γ (1 : R)))) :
    ∑ F ∈ γ.support.powerset, blockProj γ F d z = z := by
  ...
  have hcongr : ∀ (a b : MvPolynomial ι R)
      ...
      DegreeZeroLocalization.awayMk (𝓜 := natShift (polynomialGrading ι R) d)
        ...
        DegreeZeroLocalization.awayMk (𝓜 := natShift (polynomialGrading ι R) d)
          (monomial_one_mem_polynomialGrading (R := R) γ) m b hb := by
    rintro a b ha hb rfl; rfl
  calc ∑ F ∈ γ.support.powerset, blockProj γ F d
        (DegreeZeroLocalization.awayMk (𝓜 := natShift (polynomialGrading ι R) d)
          (monomial_one_mem_polynomialGrading (R := R) γ) m p hp)
      = ∑ F ∈ γ.support.powerset,
          DegreeZeroLocalization.awayMk (𝓜 := natShift (polynomialGrading ι R) d)
            (monomial_one_mem_polynomialGrading (R := R) γ) m (laurentFilter γ m F p)
            (laurentFilter_mem_natShift hp) :=
        Finset.sum_congr rfl fun F _ => blockProj_awayMk γ F hp
    _ = DegreeZeroLocalization.awayMk (𝓜 := natShift (polynomialGrading ι R) d)
          (monomial_one_mem_polynomialGrading (R := R) γ) m
          (∑ F ∈ γ.support.powerset, laurentFilter γ m F p)
          (sum_mem fun F _ => laurentFilter_mem_natShift hp) :=
        (DegreeZeroLocalization.awayMk_sum
          (monomial_one_mem_polynomialGrading (R := R) γ) m γ.support.powerset
          (fun F => laurentFilter γ m F p) fun F => laurentFilter_mem_natShift hp).symm
    _ = DegreeZeroLocalization.awayMk (𝓜 := natShift (polynomialGrading ι R) d)
          (monomial_one_mem_polynomialGrading (R := R) γ) m p hp :=
        hcongr _ _ _ _ (sum_laurentFilter_powerset γ m p)

/-- The block projections are idempotent. -/
theorem blockProj_blockProj_self [IsDomain R] (γ : ι →₀ ℕ) (F : Finset ι) (d : ℕ)
    (z : DegreeZeroLocalization (polynomialGrading ι R)
      (natShift (polynomialGrading ι R) d) (.powers (MvPolynomial.monomial γ (1 : R)))) :
    blockProj γ F d (blockProj γ F d z) = blockProj γ F d z := by
  obtain ⟨m, p, hp, rfl⟩ := DegreeZeroLocalization.exists_awayMk
    (monomial_one_mem_polynomialGrading (R := R) γ) (monomial_one_pow_ne_zero γ) z
  rw [blockProj_awayMk γ F, blockProj_awayMk γ F]
  have hcongr : ∀ (a b : MvPolynomial ι R)
      (ha : a ∈ natShift (polynomialGrading ι R) d (m • γ.degree))
      (hb : b ∈ natShift (polynomialGrading ι R) d (m • γ.degree)), a = b →
      DegreeZeroLocalization.awayMk (𝓜 := natShift (polynomialGrading ι R) d)
...
theorem blockProj_blockProj_of_ne [IsDomain R] {F F' : Finset ι} (h : F ≠ F')
    (γ : ι →₀ ℕ) (d : ℕ)
    (z : DegreeZeroLocalization (polynomialGrading ι R)
      (natShift (polynomialGrading ι R) d) (.powers (MvPolynomial.monomial γ (1 : R)))) :
    blockProj γ F d (blockProj γ F' d z) = 0 := by
  obtain ⟨m, p, hp, rfl⟩ := DegreeZeroLocalization.exists_awayMk
    (monomial_one_mem_polynomialGrading (R := R) γ) (monomial_one_pow_ne_zero γ) z
  rw [blockProj_awayMk γ F', blockProj_awayMk γ F]
...
theorem blockProj_eq_zero_of_not_subset [IsDomain R] {γ : ι →₀ ℕ} {F : Finset ι}
    (h : ¬ F ⊆ γ.support) (d : ℕ)
    (z : DegreeZeroLocalization (polynomialGrading ι R)
      (natShift (polynomialGrading ι R) d) (.powers (MvPolynomial.monomial γ (1 : R)))) :
...
theorem laurentFace_blockProj [IsDomain R] {γ γ' : ι →₀ ℕ} {e : ι} {c : ℕ}
    (hγ : γ = Finsupp.single e c + γ') (F : Finset ι) (d : ℕ)
    (z : DegreeZeroLocalization (polynomialGrading ι R)
      (natShift (polynomialGrading ι R) d) (.powers (MvPolynomial.monomial γ' (1 : R)))) :
    laurentFace hγ d (blockProj γ' F d z) = blockProj γ F d (laurentFace hγ d z) := by
  ...
  have hres : (MvPolynomial.monomial (Finsupp.single e c) (1 : R)) ^ m •
    ...
    monomial_single_pow_smul_mem (laurentFilter_mem_natShift hq)
  have hres' : (MvPolynomial.monomial (Finsupp.single e c) (1 : R)) ^ m • q ∈
      natShift (polynomialGrading ι R) d (m • (c + γ'.degree)) :=
    monomial_single_pow_smul_mem hq
  rw [blockProj_awayMk γ' F,
    laurentFace_awayMk hγ (laurentFilter_mem_natShift hq) hres,
