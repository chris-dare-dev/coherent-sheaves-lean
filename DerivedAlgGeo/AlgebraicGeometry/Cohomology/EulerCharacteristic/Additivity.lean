...
/-! ### Explicit Grothendieck group and Euler homomorphism -/

/-- The generating relations for the Grothendieck group of coherent sheaves: isomorphic
objects agree, and a short exact sequence imposes `[F₂] = [F₁] + [F₃]`. -/
inductive CoherentGrothendieckRelation (X : Variety k) :
    FreeAbelianGroup (Coh X.toScheme) → Prop
  | iso {F G : Coh X.toScheme} (e : F ≅ G) :
      CoherentGrothendieckRelation X (FreeAbelianGroup.of F - FreeAbelianGroup.of G)
  | shortExact (S : ShortComplex (Coh X.toScheme)) (hS : S.ShortExact) :
      CoherentGrothendieckRelation X
        (FreeAbelianGroup.of S.X₂ - FreeAbelianGroup.of S.X₁ - FreeAbelianGroup.of S.X₃)

/-- The subgroup generated by isomorphism and short-exact-sequence relations. -/
def coherentGrothendieckRelations (X : Variety k) :
    AddSubgroup (FreeAbelianGroup (Coh X.toScheme)) :=
  AddSubgroup.closure {z | CoherentGrothendieckRelation X z}

/-- The Grothendieck group `K₀(Coh X)`, presented explicitly by generators and relations. -/
abbrev CoherentGrothendieckGroup (X : Variety k) :=
  FreeAbelianGroup (Coh X.toScheme) ⧸ coherentGrothendieckRelations X

/-- The class of a coherent sheaf in `K₀(Coh X)`. -/
def coherentGrothendieckClass (F : Coh X.toScheme) : CoherentGrothendieckGroup X :=
  QuotientAddGroup.mk' (coherentGrothendieckRelations X) (FreeAbelianGroup.of F)

namespace FiniteCohomology

/-- The additive map on the free abelian group generated by the geometric Euler
characteristic. -/
noncomputable def freeEulerHom (D : FiniteCohomology X) :
    FreeAbelianGroup (Coh X.toScheme) →+ ℤ :=
  FreeAbelianGroup.lift D.eulerCharacteristic

/-- Euler characteristic kills every defining Grothendieck relation. -/
theorem coherentGrothendieckRelations_le_ker (D : FiniteCohomology X)
    (L : D.LinearConnectingSystem) :
    coherentGrothendieckRelations X ≤ D.freeEulerHom.ker := by
  rw [coherentGrothendieckRelations, AddSubgroup.closure_le]
  intro z hz
  change D.freeEulerHom z = 0
  cases hz with
  | iso e =>
      simp only [freeEulerHom, map_sub, FreeAbelianGroup.lift_apply_of]
      exact sub_eq_zero.mpr (D.eulerCharacteristic_iso e)
  | shortExact S hS =>
      simp only [freeEulerHom, map_sub, FreeAbelianGroup.lift_apply_of]
      rw [D.eulerCharacteristic_additive (L S hS)]
      abel

/-- The Euler characteristic as an explicit homomorphism `K₀(Coh X) → ℤ`. -/
noncomputable def grothendieckEulerHom (D : FiniteCohomology X)
    (L : D.LinearConnectingSystem) : CoherentGrothendieckGroup X →+ ℤ :=
  QuotientAddGroup.lift (coherentGrothendieckRelations X) D.freeEulerHom
    (D.coherentGrothendieckRelations_le_ker L)

@[simp]
theorem grothendieckEulerHom_class (D : FiniteCohomology X)
    (L : D.LinearConnectingSystem) (F : Coh X.toScheme) :
    D.grothendieckEulerHom L (coherentGrothendieckClass F) = D.eulerCharacteristic F := by
  simp [grothendieckEulerHom, coherentGrothendieckClass, freeEulerHom]
