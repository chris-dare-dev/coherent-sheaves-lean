...
condition and no `Stab(X)`. `RealMukai V` is a triple, not `N(X) ⊗ ℝ`, and
...
/-- Triples `(r, Δ, s)` over `ℝ`.

The real-coefficient twin of `Mukai.MukaiLattice`; see the module docstring for
why it is a twin and not a generalisation. As there, the name is suggestive of
the K3 case and no identification with a geometric lattice is made. -/
abbrev RealMukai (V : Type*) : Type _ := ℝ × V × ℝ

/-- The Mukai pairing over `ℝ`, built from a bilinear form `q` on the middle
summand. Compare `Mukai.pairing`. -/
def pairing (v w : RealMukai V) : ℝ :=
  q v.2.1 w.2.1 - v.1 * w.2.2 - w.1 * v.2.2
...
/-- `⟪v, v⟫`. Compare `Mukai.selfPairing`. -/
def selfPairing (v : RealMukai V) : ℝ := pairing q v v

theorem selfPairing_eq_pairing (v : RealMukai V) :
...
below. Both are elements of `RealMukai V`; the pairing against a real class is
...
noncomputable def chartRe (β ω : V) : RealMukai V :=
  (1, β, (q β β - q ω ω) / 2)

/-- The imaginary part of `exp(β + iω)`. -/
def chartIm (β ω : V) : RealMukai V :=
  (0, ω, q β ω)

/-- `Re (℧, δ)` for `℧ = exp(β + iω)`. Inherits `noncomputable` from
`chartRe`. -/
noncomputable def pairingRe (β ω : V) (δ : RealMukai V) : ℝ :=
  pairing q (chartRe q β ω) δ

/-- `Im (℧, δ)` for `℧ = exp(β + iω)`. -/
def pairingIm (β ω : V) (δ : RealMukai V) : ℝ :=
  pairing q (chartIm q β ω) δ

theorem pairingRe_eq (β ω : V) (δ : RealMukai V) :
    pairingRe q β ω δ = q β δ.2.1 - δ.2.2 - δ.1 * ((q β β - q ω ω) / 2) := by
  simp only [pairingRe, pairing, chartRe, one_mul]

theorem pairingIm_eq (β ω : V) (δ : RealMukai V) :
    pairingIm q β ω δ = q ω δ.2.1 - δ.1 * q β ω := by
  simp only [pairingIm, pairing, chartIm, zero_mul, sub_zero]
...
theorem two_mul_rk_mul_pairingRe (hq : ∀ x y : V, q x y = q y x) (β ω : V)
  ...
  rw [pairingRe_eq, selfPairing_eq_pairing, pairing]
  simp only [map_sub, map_smul, LinearMap.sub_apply, LinearMap.smul_apply,
    smul_eq_mul]
  rw [hq δ.2.1 β]
  ring
...
theorem pairingRe_of_rk_eq_zero (β ω : V) {δ : RealMukai V} (hr : δ.1 = 0) :
...
def IsSpherical (δ : RealMukai V) : Prop := selfPairing q δ = -2

theorem isSpherical_iff (δ : RealMukai V) :
    IsSpherical q δ ↔ selfPairing q δ = -2 :=
  Iff.rfl

/-- `Δ⁺`: the spherical classes of positive rank. -/
def sphericalPlus : Set (RealMukai V) :=
  {δ | IsSpherical q δ ∧ 0 < δ.1}

theorem mem_sphericalPlus_iff (δ : RealMukai V) :
    δ ∈ sphericalPlus q ↔ IsSpherical q δ ∧ 0 < δ.1 :=
  Iff.rfl

theorem isSpherical_of_mem_sphericalPlus {δ : RealMukai V}
    (h : δ ∈ sphericalPlus q) : IsSpherical q δ :=
  h.1

theorem rk_pos_of_mem_sphericalPlus {δ : RealMukai V}
...
theorem corank_eq_of_isSpherical {δ : RealMukai V} (hs : IsSpherical q δ)
    (hr : δ.1 ≠ 0) :
...
def wall (δ : RealMukai V) : Set (V × V) :=
  {p | pairingIm q p.1 p.2 δ = 0 ∧ pairingRe q p.1 p.2 δ ≤ 0}

theorem mem_wall_iff (δ : RealMukai V) (p : V × V) :
...
def chamber (S : Set (RealMukai V)) : Set (V × V) :=
  {p | ∀ δ ∈ S, p ∉ wall q δ}

theorem mem_chamber_iff (S : Set (RealMukai V)) (p : V × V) :
    p ∈ chamber q S ↔ ∀ δ ∈ S, p ∉ wall q δ :=
  Iff.rfl

theorem chamber_antitone {S T : Set (RealMukai V)} (h : S ⊆ T) :
    chamber q T ⊆ chamber q S :=
  fun _ hp δ hδ => hp δ (h hδ)

theorem chamber_eq_compl_iUnion (S : Set (RealMukai V)) :
...
theorem mem_wall_iff_of_isSpherical (hq : ∀ x y : V, q x y = q y x)
    {δ : RealMukai V} (hs : IsSpherical q δ) (hr : 0 < δ.1) (p : V × V) :
    p ∈ wall q δ ↔
...
def IntegralComparison.map (c : IntegralComparison q b)
    (v : Mukai.MukaiLattice N) : RealMukai V :=
...
theorem pairing_map (c : IntegralComparison q b) (v w : Mukai.MukaiLattice N) :
  ...
  simp only [pairing, IntegralComparison.map, Mukai.pairing, c.compat]
  push_cast
  ring
