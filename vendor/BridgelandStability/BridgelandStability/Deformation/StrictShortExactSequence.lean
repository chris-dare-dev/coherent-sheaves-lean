...
theorem exists_first_strictShortExact_of_not_semistable_of_finite_leftHeartSubobjects
  ...
  have hT_fin : T.Finite := by
    rw [Set.finite_def]
    change Nonempty (Fintype {M : Subobject X // M ≠ ⊥ ∧ IsStrictMono M.arrow})
    letI : Finite {M : Subobject X // M ≠ ⊥ ∧ IsStrictMono M.arrow} :=
  ...
  · by_cases hXZ : IsZero X
    ...
    · let xE₂ : XI₂ ⟶ EI₂ := xK ≫ iK
      ...
      have hmonoRH :
        ...
        let FR₁ := Slicing.IntervalCat.toRightHeart (C := C) (s := σ.slicing) a₁ b
          (Fact.out : b - a₁ ≤ 1)
        let FR₂ := Slicing.IntervalCat.toRightHeart (C := C) (s := σ.slicing) a₂ b
          (Fact.out : b - a₂ ≤ 1)
