...
noncomputable def interval_cokernel_pullbackTopIso
  ...
  have heK : eK.hom ≫ kernel.ι p = pb.arrow := by
    change (IsLimit.conePointUniqueUpToIso (kernelIsKernel p) hKer).inv ≫
      kernel.ι p = pb.arrow
  ...
  · haveI : Mono xQ := hS₁.shortExact.mono_f
    ...
    have hsumL :
      ...
      have hsumL' :
        ...
        simpa [ssf₂, StabilityCondition.WithClassMap.skewedStabilityFunction_of_near,
          SL, PB, KI₂, map_add] using
