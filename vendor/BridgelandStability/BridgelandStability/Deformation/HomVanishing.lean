...
theorem StabilityCondition.WithClassMap.hom_eq_zero_of_deformedPred
  ...
  · -- Small gap: 0 < ψ₁ - ψ₂ ≤ 2ε₀.
    ...
    · set a : ℝ := (ψ₁ + ψ₂) / 2 - 1 / 2
      ...
      have hT_I' : Triangle.mk (ι.map i_I) (ι.map πQH) δ_I ∈ distTriang C := hT_I
      have hT_pH' :
          Triangle.mk (ι.map (Triangulated.AbelianSubcategory.ιK f₃ α))
            (ι.map pH) (-m₃) ∈ distTriang C := hT_pH
      ...
      have hpL_epi : Epi (FL.map pL) := by
        let eE : EH ≅ FL.obj EL := ObjectProperty.isoMk _ (Iso.refl E)
        let eI : I_H ≅ FL.obj IL := ObjectProperty.isoMk _ (Iso.refl I_H.obj)
        letI : Epi (eE.inv ≫ pH ≫ eI.hom) := inferInstance
        have hEq : FL.map pL = eE.inv ≫ pH ≫ eI.hom := by
          ext
          dsimp [FL, pL, EL, IL, eE, eI]
      ...
      have hI_phase_ge_left :
        ...
        have hphase := SkewedStabilityFunction.phase_le_of_strictQuotient
            (C := C) (σ := σ) (a := a) (b := ψ₁ + ε₀) (ssf := ssfL)
            (X := EL) (Y := IL) hSS₁_left hε₀ hleftThin hW_interval_left hpert_left pL
            hpL_strict hIne
        simpa [ssfL, StabilityCondition.WithClassMap.skewedStabilityFunction_of_near,
          αL, EL, IL, pL] using hphase
      ...
      have hI_phase_le_big :
        ...
        have hphase := hSS₂_big.le_of_distTriang hT_I' hI_big hQ_big hIne
        rw [hι_simp I_H] at hphase
        simpa [StabilityCondition.WithClassMap.skewedStabilityFunction_of_near] using hphase
