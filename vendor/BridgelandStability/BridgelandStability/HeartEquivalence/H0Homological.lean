...
noncomputable def TStructure.shortComplexOfDistTriangle_map_truncGEIsoOfSplit
  ...
  refine ShortComplex.isoMk
    ...
    e₂
    (Iso.refl _) ?_ ?_
  · change (t.truncGE 0).map ((t.truncGEπ 0).app X₁) ≫ (t.truncGE 0).map m₁ =
      (t.truncGE 0).map f ≫ (t.truncGE 0).map v
    simpa only [Functor.map_comp] using congrArg ((t.truncGE 0).map) hm₁
  · simpa [e₂, Functor.map_comp] using congrArg ((t.truncGE 0).map) hm₃
...
theorem HeartStabilityData.H0primeFunctor_preadditiveCoyoneda_exact_of_isIso_truncLT_map
  ...
  have hu' :
    ...
    have hu₂ :
      ...
      simp only [u', Category.assoc, IsIso.inv_hom_id_assoc]
...
theorem HeartStabilityData.H0primeFunctor_preadditiveYoneda_isHomological_of_eval
  ...
  intro E
  change ((shortComplexOfDistTriangle T hT).map
    (h.H0primeFunctor (C := C) ⋙
      preadditiveCoyoneda.obj (Opposite.op (Opposite.unop E)))).Exact
  exact hExact T hT (Opposite.unop E)
...
The component-qualified suffix avoids a declaration-name collision with the
independent implementation in `CohLean`. -/
noncomputable def SpectralObject.mapHomologicalFunctorBridgelandStability
  ...
  exact₁' n₀ n₁ h D := by
    ...
    exact ComposableArrows.exact₂_mk _ _ hEx
  exact₂' n D := by
