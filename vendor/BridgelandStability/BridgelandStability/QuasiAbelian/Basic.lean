  ...
  exact ⟨n, fun m hm ↦
    subobjectImageOfFaithfulPreservesMono_injective (A := A) (C := C) F (E := E)
      (by
        ...
        change subobjectImageOfFaithfulPreservesMono (A := A) (C := C) F (E := E) (f n) =
          subobjectImageOfFaithfulPreservesMono (A := A) (C := C) F (E := E) (f m) at h
        exact h)⟩
          ...
          exact ⟨n, fun m hm ↦
            strictSubobjectImageOfFaithful_injective (A := A) (C := C) F hF (E := E)
              (by
                have h := congrArg
                  (fun S : (Subobject (F.obj E))ᵒᵈ => OrderDual.ofDual S) (hn m hm)
                change strictSubobjectImageOfFaithful (A := A) (C := C) F hF (E := E) (f n) =
                  strictSubobjectImageOfFaithful (A := A) (C := C) F hF (E := E) (f m) at h
                exact h)⟩))
