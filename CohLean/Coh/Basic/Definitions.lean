...
cokernels in `X.Modules` is proved in `CohLean.Coh.Abelian.Kernels`; closure under extensions is
proved in `CohLean.Coh.Abelian.Extensions`. `CohLean.Coh.Abelian.Basic` installs the abelian instance and
packages the inclusion as an exact functor.

The affine equivalence `Coh (Spec R) ≌ FGModuleCat R` is proved in `CohLean.Coh.Affine.Comparison`.

Downstream developments should import `CohLean.Coh.Abelian.Basic` for the completed stage-1 API.
