...
namespace CohLean.Intersection.ChernCharacterSurface
...
resolution. -/
def virtualRank {F : Coh X.toScheme} (P : Coh.TwoTermPerfectDeterminantData F) : ℤ :=
  (P.middle.rank : ℤ) - (P.left.rank : ℤ)

/-- The determinant first Chern class, in the additive form of the Picard group. -/
def picardFirstChernClass {F : Coh X.toScheme}
    (P : Coh.TwoTermPerfectDeterminantData F) : Additive (Pic X.toScheme) :=
...
theorem numericalFirstChernClass_ofIso
    {D : FiniteCohomology X} {C : D.LinearConnectingSystem}
    (I : IntersectionContext D C 2) {F G : Coh X.toScheme}
    (P : Coh.TwoTermPerfectDeterminantData F) (e : F ≅ G) :
...
noncomputable def toddOnePairing
    {D : FiniteCohomology X} {C : D.LinearConnectingSystem}
    (I : IntersectionContext D C 2) : Additive (Pic X.toScheme) →+ ℚ where
  toFun L :=
...
theorem toddOnePairing_apply
    {D : FiniteCohomology X} {C : D.LinearConnectingSystem}
    (I : IntersectionContext D C 2) (L : Additive (Pic X.toScheme)) :
    toddOnePairing I L =
      ((I.eulerPic L.toMul - I.eulerPic 1 : ℤ) : ℚ) -
        (I.surfaceIntersectionPairing L L : ℤ) / 2 :=
  rfl
...
@[ext]
structure SurfaceChernCharacter (X : Variety k) where
  rank : ℤ
  first : Additive (Pic X.toScheme)
  secondDegree : ℚ
...
noncomputable def surfaceChernCharacter
    {D : FiniteCohomology X} {C : D.LinearConnectingSystem}
    (I : IntersectionContext D C 2) {F : Coh X.toScheme}
    (P : Coh.TwoTermPerfectDeterminantData F) : SurfaceChernCharacter X where
  rank := virtualRank P
...
theorem numericalFirstChernClass_shortExact
    {D : FiniteCohomology X} {C : D.LinearConnectingSystem}
    (I : IntersectionContext D C 2) {S : ShortComplex (Coh X.toScheme)}
...
theorem chernCharacterTwoDegree_lineBundle
    {D : FiniteCohomology X} {C : D.LinearConnectingSystem}
    (I : IntersectionContext D C 2) {F : Coh X.toScheme}
    (P : Coh.TwoTermPerfectDeterminantData F) (L : Pic X.toScheme)
    (hrank : virtualRank P = 1)
...
end CohLean.Intersection.ChernCharacterSurface
